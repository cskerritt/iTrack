...
export const FINANCE_CERTIFICATION_RULE_SET_SEED_BINDINGS = [
  ruleSet({
    ...
    sourceNote: `IMA Rules and Requirements for Maintaining Certifications for an established CMA calendar year: complete 30 qualifying CPE hours by December 31, including two hours of ethics, maintain IMA membership in good standing, and keep the IMA transcript current. The ethics hours are included within the 30-hour total. Up to 10 excess CPE hours may apply to the next calendar year, but License Lantern never copies carryover automatically; record only the eligible amount supported by the prior-year record. CPE begins under IMA's award-year timing rules, so use this template for a full reporting year shown in the account rather than assuming a partial initial year. Retired status, hardship waivers, delinquency remediation, CFM maintenance, and any employer or jurisdictional requirements are separate paths. ${PRIVATE_CREDENTIAL_SCOPE}`,
  }),
  ruleSet({
    id: "acfe-cfe-2026-v1",
    stableKey: "acfe-cfe",
    ...
    sourceNote: `ACFE CPE requirements for an established CFE annual compliance period: earn 20 accepted CPE credits every 12 months, including at least 10 fraud-related credits and two ethics-related credits, pay annual membership dues, and certify compliance by the end of the holder's anniversary month. Ethics credit may also qualify as fraud-related when ACFE's field-of-study rules support both classifications. Retain acceptable proof for three years; ACFE does not maintain the complete record for the holder. Up to 10 excess credits may carry into the immediately following period only after the current 20-credit, fraud, and ethics requirements are met; License Lantern never copies carryover automatically. New CFEs are exempt through their next anniversary month, so the initial exemption period is not this template. A 30-day extension or additional discretionary relief must be approved by ACFE and is not assumed. ${PRIVATE_CREDENTIAL_SCOPE}`,
  }),
  ruleSet({
