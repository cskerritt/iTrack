...
- **Type:** display **Bricolage Grotesque** (static 700/800 instances with the optical-size axis pinned at 96 at EVERY size, the 13–17 px ring labels included — a visible deviation from the mockup artboards, which rendered the variable font at auto optical size; a live opsz axis alone exceeds the 40 kB budget, and a text-cut instance is a Wave 4 follow-up if Chris wants it; tight tracking) for page titles, hero numerals, and ring labels; body **Atkinson Hyperlegible** (legibility-first, a deliberate choice for a compliance audience and the a11y findings); data **IBM Plex Mono** for dates, credit counts in tables, ids, tabular numerals everywhere digits align (shipped as "iTrack Mono": the subset is a Modified Version under the OFL and may not carry the Reserved Font Name "Plex"). Fonts self-hosted under `public/fonts/` as woff2 subsets, preloaded, `font-display: swap`, a 40 kB per-face budget (perf-08). Type scale in `rem` (a11y-14): 0.75, 0.8125, 0.875, 1, 1.125, 1.375, 1.75, 2.25, 3.
