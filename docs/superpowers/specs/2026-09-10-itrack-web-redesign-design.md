...
- **Verification flow redesigned as one unit** (landing-auth-05, security-03, landing-auth-M-02, security-M-01, security-04, infra-M-02): `GET /verify?token=` renders a page with a single "Confirm my email" **POST** button; the token is consumed only on POST; success signs the user in and redirects to `/`. Signing up again with an email that has an **unverified** account replaces the stored name and password hash and re-sends the link (so a squatter's password never survives). Signup, login-unverified, resend and reset all end on the same neutral copy: "If that address can be used, we've sent an email to it." Login with wrong credentials or unverified account returns the same generic error; the resend control lives on the signup "sent" page and on the login page as a POST form with its own email field (fixes landing-auth-06/M-05).
